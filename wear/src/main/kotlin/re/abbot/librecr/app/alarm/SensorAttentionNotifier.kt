...
/**
 * Watch-side one-shot status notification when the sensor needs the user to act (replace / ended /
 * unknown error). Called only on attention transitions, so it fires once per change and clears
 * itself when the sensor returns to normal.
 */
object SensorAttentionNotifier {
    ...
    private fun message(attention: Libre3SensorAttention): Pair<String, String> = when (attention) {
        Libre3SensorAttention.ReplaceSensor ->
            "Înlocuiește senzorul" to "Senzorul trebuie înlocuit."
        Libre3SensorAttention.SensorEnded ->
            "Senzor încheiat" to "Sesiunea senzorului s-a încheiat."
        is Libre3SensorAttention.Unknown ->
            "Eroare senzor" to "Cod necunoscut (${attention.code})."
        else -> "Senzor" to ""
    }
